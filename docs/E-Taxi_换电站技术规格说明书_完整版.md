...
- 论文写法将未换成功车辆直接计入下一时刻状态转移项。  
- 当前实现引入 `waiting_queue`：未换成功车辆先进入站内等待队列，下一时段继续尝试换电。  
- 因此仿真中的车辆守恒按 `vacant + occupied + waiting` 核验。
...
- `charging_solver=gurobi`：可抢占（preemptive）且可迁移（migratable）。  
- `charging_solver=fcfs`：非抢占（non-preemptive），作为站端基线。  
- `charging_solver=edf`：按截止时间优先调度，可抢占。

这意味着同一实验中应明确区分“优化策略”和“基线策略”的可抢占假设，避免口径混淆。
...
**实现说明**：当前 Gurobi 充电器在工程实现中使用软约束
$$
\sum_{t=A_i}^{D_i-1} x_i^t + s_i = C_i,\quad s_i \ge 0
$$
并在目标函数中惩罚 `miss_penalty * Σ s_i`，用于避免资源不足时模型直接不可行。
...
```
论文 Eq. (14): Σ_{t=A_i}^{D_i-1} x_i^t = C_i
实现（软约束）: Σ_{t=A_i}^{D_i-1} x_i^t + s_i = C_i, s_i ≥ 0
