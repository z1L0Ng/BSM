...
OUTPUT_CSV="${2:-results/baseline_combinations/stress_gate48_6combo.csv}"
...
TMP_DIR="${OUT_DIR}/_tmp_stress_gate48"
mkdir -p "${TMP_DIR}"
...
if not rows:
    raise RuntimeError("No rows were generated from stress 48-run gate sweep.")
...
print(f"Saved 48-run gate matrix to: {output_csv}")
